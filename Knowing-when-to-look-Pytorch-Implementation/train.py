...
from utils import clip_gradient, save_checkpoint, adjust_learning_rate, accuracy, AverageMeter
...
attention_dim = 49  # attention_dim = k = num_pixels, i.e 7x7
hidden_size = 512  # decoder hidden层维数
dropout = 0.5
...
batch_size = 64
workers = 1  # 多处理器加载数据,如果是windows则设置为1,否则设置为0
...
fine_tune_encoder = True  # 是否微调
checkpoint = 'BEST_checkpoint_flickr8k.pth'  # 模型断点, 无则None
...
def main():
    ...
    if checkpoint is None:
        decoder = DecoderWithAttention(hidden_size=hidden_size,                                   vocab_size=len(word_map), 
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch, vocab_size):
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        # 前向计算
        spatial_f, global_f, enc_image = encoder(imgs)
        predictions, alphas, betas, encoded_captions, decode_lengths, sort_ind = decoder(spatial_f, global_f, enc_image, caps, caplens)
        ...
        # scores 维度变成两维 shape [所有没有pad的元素, vocab_size]
        # targets shape [所有没有pad的元素]
        # 注意: Pytorch version: 1.1.0
...
def validate(val_loader, encoder, decoder, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            # allcaps shape: [batch_size, captions_per_image, max_cap_len]
            ...
            # preds返回下标 (batch_size, max_cap_len, 1)
